...
<!--  <van-nav-bar :title="$t($route.meta.title as string)" :left-arrow="!tabbarVisible" @click-left="goBack" />-->
  <nut-tabbar unactive-color="#364636" active-color="#1989fa" top v-model="activeTab" v-show="tabbarVisible" @tab-switch="tabSwitch">
    <nut-tabbar-item v-for="item in tabItem" :key="item.key" :tab-title="$t(`tabbar.${item.key}`)" />
<!--    :icon="item.icon"-->
  </nut-tabbar>
